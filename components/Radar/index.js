...
class Radar extends Component {
    ...
    initRadar = () => {
        ...
        this.radarDataGraph(data, centreCircle, radiusArray, 20, '#7cefe4');
    }
    ...
    //绘制数据图 radarDataGraph(数据数组,圆心坐标,雷达图半径数组,边数,数据区域填充颜色)
    radarDataGraph = (data, centreCircle, radiusArray, n, fillColor) => {
    ...
    //绘制雷达网radarBasePage(圆心坐标,雷达图半径数组,边数)
    radarBasePage = (centreCircle, radiusArray, n) => {
        var canvas = document.getElementById("myCanvas");
        var ctx = canvas.getContext("2d");
        //抗锯齿效果
        var width = canvas.width;
    ...
    //绘制圆点
    radarCircle = (point, r, fillColor, lineWidth) => {
        var canvas = document.getElementById("myCanvas");
        var ctx = canvas.getContext("2d");
        ctx.fillStyle = fillColor;
        ctx.moveTo(point.x, point.y);
    ...
    render(){
        return(
            <div className="radar">
                ...
                <div className="btn" onClick={this.setRevolve}>{this.state.isRevolve ? "停止旋转" : "开始旋转"}</div>
                <style jsx>{`
                    .radar{
                        display:flex;
